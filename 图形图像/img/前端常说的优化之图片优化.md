...
- sizes： 必须结合 srcset 属性使用；其值也是一组值，通过不同的媒体条件，指定最合适的图片；
...
/>
```
在 1x 的屏幕上显示 1.png； 在 2x 的屏幕上显示 2.png; 在 3x 的屏幕上显示 3.png。其中 *nx* 指的是 **DPR** -- 设备像素率 (Device Pixel Ratio)。
...
如果使用ipad（屏幕宽度768px， DPR=2）访问，同样满足 `(max-width: 800px) 200px`， 同样会获得期望css尺寸 200px， 因为DPR = 2，于是，显示一个 srcset 中 200 × 2 = 400w 的图。
