...
基本原理，就是在 git 的 `commit-msg` 钩子中去判断提交的信息是否合法。

除了自己在`.git/hooks`下手写shell脚本这种简单原始的方法外，市面上还有多种解决方案，大概看了一下后，我觉得还得去看看大牛是咋搞的，所以，还是以 Vue 项目为例：

原来（V2.5.14前）的 Vue 是这么干的：
1. 把检查脚本放到 `build/git-hooks/commit-msg` 下
    ```sh
    ...
    ```
2. 克隆仓库后，运行 `npm install & npm run setup`，在 `.git/hooks` 下创建脚本的链接
    ```JavaScript
...
```JSON
"gitHooks": {
    "commit-msg": "node build/git-commit-msg.js"
}
```
在 build 目录下创建 git-commit-msg.js 文件：
